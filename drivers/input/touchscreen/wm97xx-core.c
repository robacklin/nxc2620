...
static void wm97xx_pen_irq_worker(struct work_struct *work)
{
	...
	if (!wm->pen_is_down && wm->mach_ops->acc_enabled && wm->mach_ops->acc_pen_up)
...
static int wm97xx_init_pen_irq(struct wm97xx *wm)
{
	...
	if (request_irq(wm->pen_irq, wm97xx_pen_interrupt, IRQF_TRIGGER_RISING /*IRQF_SHARED*/,
			"wm97xx-pen", wm)) {
		dev_err(wm->dev,
	...
	if (wm->id != WM9705_ID2) 
	{
		...
		reg = wm97xx_reg_read(wm, 0x4c);
		wm97xx_reg_write(wm, 0x4c, reg & ~(wm->mach_ops->irq_gpio));
...
static int wm97xx_read_samples(struct wm97xx *wm)
{
	...
	mutex_lock(&wm->codec_mutex);
#if 0
	if (wm->mach_ops && wm->mach_ops->acc_enabled && wm->mach_ops->acc_pen_down)
	...
	if (rc & RC_PENUP) {
		...
		dev_dbg(wm->dev,
			"pen down[%x]: x=%x:%d, y=%x:%d, pressure=%x:%d\n",
			wm->pen_is_down,
